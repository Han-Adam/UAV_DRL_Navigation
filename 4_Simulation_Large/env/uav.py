...
class UAV(object):
    def __init__(self,
        ...
        self.PWM_MAX = param_dict['PWM_MAX']
        self.MOTOR_POW_MIN = self.MOTOR_BIAS ** 2
        self.MOTOR_POW_MAX = (self.MOTOR_COEFFICIENT*self.PWM_MAX + self.MOTOR_BIAS) ** 2
    ...
    def apply_action(self, F, t):
        ...
        mp_min = np.min(motor_speed_d_pow)
        motor_speed_d_pow += (self.MOTOR_POW_MIN - mp_min) if mp_min < self.MOTOR_POW_MIN else 0
        mp_max = np.max(motor_speed_d_pow)
        if mp_max > self.MOTOR_POW_MAX:
            motor_speed_d_pow = (self.MOTOR_POW_MAX - self.MOTOR_POW_MIN) / (mp_max - self.MOTOR_POW_MIN) \
                                * (motor_speed_d_pow - mp_max) + self.MOTOR_POW_MAX
