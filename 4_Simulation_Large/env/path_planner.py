...
class PathPlanner:
    def __init__(self, r, g, b, target_pos):
        ...
        self.g = g
        self.b = b
        self.target_pos = np.array(np.floor(target_pos * 5) + [75, 25, 0] + [20, 20, 20], dtype=np.compat.long)

    def get_path(self, map3d, pathmap, current_pos):
        current_pos = np.array(np.floor(current_pos * 5) + [75, 25, 0] + [20, 20, 20], dtype=np.compat.long)
        for i in range(5):
    ...
    def _get_s(self, map3d, current_pos):
        ...
        s_error = (0.2 * s_error)
        # s_error = np.clip(s_error, -1.4, 1.4)

        bound = 5
        norm = np.max(np.abs(s_error))
        if norm>bound:
            s_error *= (bound/norm)
        s_error = np.floor(s_error*5)*0.2
