...
class Env:
    ...
    def reset(self, initial=None, target=None):
        ...
        if initial is None:
            while 1:
                x = np.random.randint(10, 60) # [low, high)
    ...
    def show_map(self, path):
        ...
        x = 0.2 * np.outer(np.cos(u), np.sin(v))
        y = 0.2 * np.outer(np.sin(u), np.sin(v))
        z = 0.2 * np.outer(np.ones(np.size(u)), np.cos(v))

        initial_pos = 0.2 * self.initial_pos - np.array([6.9, 6.9, 1.9])
        ...
        target_pos = 0.2 * self.target_pos - np.array([6.9, 6.9, 1.9])
